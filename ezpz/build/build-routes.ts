    ...
    const generateImportsForSSR = async (routes_object: any) => {
      ...
      for (let i = 0; i < routes_array.length; i++) {
        if (typeof routes_array[i][1] === 'string') {
          ...
          // we're a string, so we're a route
          server_side_render_imports.push(`import * as ${component_name} from ${component_file_path}`
          )
        } else {
          // we're an object, so we're a directory
...
// router array
export const routes: RouteSSR[] = [
  ${react_router_routes.map((x) => `{
    name: '${x.Component}',
    path: '${x.path}',
    Component: ${x.Component}.default,
    config: ${x.Component}.config,
    loadFunctionData: ${x.Component}.loadFunctionData,
  }`).join(',\n  ')}
]
`
